...
namespace SMBLibrary.Server
{
    public partial class SMBServer
    {
        ...
        public SMBServer(GSSProvider securityProvider):this(new SMBShareCollection(), securityProvider)
        {
        }
        ...
        public void AddShare(FileSystemShare fss)
        {
            this.m_shares.Add(fss);
        }
        ...
        private void ReceiveCallback(IAsyncResult result)
        {
            ...
            try
            {
                ...
                numberOfBytesReceived = clientSocket.EndReceive(result, out errorCode);
                if (errorCode != SocketError.Success)
                {
                    const int WSAECONNRESET = 10054;
                    if (((int)errorCode) == WSAECONNRESET)
                    {
                        state.LogToServer(Severity.Debug, "The connection was forcibly closed by the remote host");
                    }
                    else
                    {
                        state.LogToServer(Severity.Debug, "The connection was terminated, Socket error code: {0}", errorCode);
                    }
